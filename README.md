...
| [OCR-DOTA-V3/](OCR-DOTA-V3/) | **V3 主实现**：`ocr_dota_v3/`（rank_compatibility.py、model.py）、configs、单元测试、run.py、smoke_cache.py、tune_\*/supervise_\* 调参驱动、BEST\_\*.json/.txt 冠军结果；`run_core2_ablation10.py` 双核消融（十数据集，最终结果在 `log/core2_ablation10_20260910_rankfree/`，含 FINAL_REPORT.md）；`run_dataset_specific_oracle10.py` 逐数据集 oracle 调参（结果在 `log/core2_dataset_specific_oracle10_20260910/`，Posterior 6/10、Full 7/10 数据集为正，EuroSAT responsibility +740） |
