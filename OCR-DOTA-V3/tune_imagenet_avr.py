#!/usr/bin/env python3
"""Fixed-order full-stream OCR-DOTA V3 tuning for ImageNet-A/R/V."""
...
VERSION = "ocr-dota-v3-imagenet-avr-fixed-order-v2"
DATASETS = ("A", "R", "V")
STAGES = ("initial", "stage2", "stage3")
DEFAULT_OUTPUT = REPO_ROOT / "log/all_dataset_perf/ocr_dota_v3_imagenet_avr_fixed_order_24h_20260901"
ORDER_SOURCE = REPO_ROOT / "BEST_SINGLE_CONFIG_AVR_ORDER_TUNED.json"
LATEST_SOURCE = REPO_ROOT / "BEST_SINGLE_CONFIG_ALL41_AFTER_8_ROUNDS.json"
...
def load_anchors(order_source: Path, latest_source: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    order_payload = json.loads(order_source.read_text(encoding="utf-8"))
    order_table = order_payload.get("datasets", order_payload)
    latest_payload = json.loads(latest_source.read_text(encoding="utf-8"))
    latest_table = latest_payload.get("datasets", latest_payload)
    anchors: dict[str, Any] = {}
    provenance: dict[str, Any] = {}
    for dataset in DATASETS:
        identity_row = order_table[dataset]
        config_row = latest_table[dataset] if dataset == "V" else identity_row
        if identity_row.get("sample_order_seed") != REGISTRY[dataset]["sample_order_seed"]:
            raise RuntimeError(f"{dataset} anchor seed mismatch")
        if identity_row.get("sample_order_sha256") != REGISTRY[dataset]["sample_order_sha256"]:
            raise RuntimeError(f"{dataset} anchor order SHA mismatch")
        if identity_row.get("cache_sha256") != REGISTRY[dataset]["cache_sha256"]:
            raise RuntimeError(f"{dataset} anchor cache SHA mismatch")
        migrated = core.migrate_anchor(dataset, unwrap_config(config_row))
        anchors[dataset] = migrated
        provenance[dataset] = {
            "config_source": str(latest_source if dataset == "V" else order_source),
            "config_correct": int(config_row.get("correct", -1)),
            "identity_source": str(order_source),
...
def generate_stage(dataset: str, stage: str, incumbent: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    if stage == "initial":
        specs = core.build_initial_candidates(cfg)
        a0 = next(item["config"] for item in specs if item["candidate_id"] == "A0")
        specs.append(spec(a0, "G0_residual_0", candidate_id="G0", update__residual_strength=0.0))
    elif stage == "stage2":
        specs.extend(spec(cfg, f"S2_residual_{value:g}", update__residual_strength=value) for value in (0., .5))
        beta = float(cfg["update"]["beta"])
        specs.extend(spec(cfg, f"S2_beta_x{factor:g}", update__beta=min(1., max(0., beta * factor))) for factor in (.75, 1.25))
        count = float(cfg["update"]["init_count"])
        specs.extend(spec(cfg, f"S2_count_x{factor:g}", update__init_count=count * factor) for factor in (.75, 1.25))
    elif stage == "stage3":
        for axis in ("eta", "rho"):
            for factor in (.9, 1.1):
                specs.append(spec(cfg, f"S3_{axis}_x{factor:g}", **{f"base__{axis}": float(cfg["base"][axis]) * factor}))
    else:
...
def write_summary(
    ...
    if rows:
        ...
        lines = ["OCR-DOTA-V3 ImageNet-A/R/V fixed-order tuning oracle", ""]
        for dataset in datasets:
            if dataset in rows:
                row = rows[dataset]
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--order-source", default=str(ORDER_SOURCE))
    parser.add_argument("--latest-source", default=str(LATEST_SOURCE))
    ...
    parser.add_argument("--verification-reserve-hours", type=float, default=1.0)
...
def main() -> int:
    ...
    order_source, latest_source = Path(args.order_source).resolve(), Path(args.latest_source).resolve()
    anchors_all, provenance_all = load_anchors(order_source, latest_source)
    ...
    identity_record = build_identity(
        ...
        source_shas={"order": core.sha256_file(order_source), "latest": core.sha256_file(latest_source),
    ...
    if args.dry_run:
        plan = {dataset: {stage: len(generate_stage(dataset, stage, {"config": anchors[dataset]})) for stage in stages} for dataset in datasets}
        print(json.dumps({"identity_sha256": identity_sha, "candidate_counts_from_anchor": plan, "caches": cache_records}, indent=2))
        return 0
    ...
    with core.PidLock(out / "RUNNING.pid"):
        try:
            ...
            for stage_index, stage in enumerate(stages):
                ...
                for dataset in datasets:
                    if stage == "initial":
                        ...
                        continue
                    initial_info = stage_info[dataset].get("initial")
                    if not initial_info or initial_info.get("status") != "complete" or int(initial_info.get("gain_correct", 0)) < 1:
                        block = jobs["datasets"].setdefault(dataset, {"stages": {}, "stage_results": {}})
                        block["stage_results"].setdefault(stage, {"status": "skipped_no_gain", "required_stage": "initial"})
                        stage_info[dataset].setdefault(stage, block["stage_results"][stage])
                        continue
                    if stage == "stage3":
                        stage2_info = stage_info[dataset].get("stage2")
                        if not stage2_info or stage2_info.get("status") != "complete" or int(stage2_info.get("gain_correct", 0)) < 1:
                            block = jobs["datasets"].setdefault(dataset, {"stages": {}, "stage_results": {}})
                            block["stage_results"].setdefault(stage, {"status": "skipped_no_gain", "required_stage": "stage2"})
                            stage_info[dataset].setdefault(stage, block["stage_results"][stage])
                            continue
                    eligible.append(dataset)
                save_jobs()
                for dataset_index, dataset in enumerate(eligible):
                    ...
                    if stage_block is None:
                        ...
                        stage_block = {"incumbent_fingerprint": core.fingerprint(dataset, incumbent["config"]), "candidates": candidates}
                    ...
                    if len(stage_rows) != len(candidates):
                        ...
                        break
                    best = core.select_best(stage_rows, anchors[dataset])
                    if stage == "initial":
                        a0 = next(row for row in stage_rows if row["candidate_id"] == "A0")
                        incumbent, reference = best, int(a0["correct"])
                    else:
            ...
            final_datasets = [dataset for dataset in datasets if stage_info[dataset].get("initial", {}).get("status") == "complete"]
            ...
            if formal_complete and set(datasets) == set(DATASETS):
                payload = {"version": VERSION, "status": status, "protocol": "A seed152 / V seed20 fixed-order oracle; R canonical-order oracle",
                           "datasets": winners, "summary_sha256": final_manifest["summary_sha256"]}
                core.atomic_json(V3_ROOT / "BEST_SINGLE_CONFIG_IMAGENET_AVR.json", payload)
                ...
                (V3_ROOT / "BEST_SINGLE_CONFIG_IMAGENET_AVR.txt").write_text("\n".join(lines) + "\n", encoding="utf-8")
            return 0
