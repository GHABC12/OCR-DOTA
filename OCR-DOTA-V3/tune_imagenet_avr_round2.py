#!/usr/bin/env python3
"""Second-round fixed-order OCR-DOTA V3 tuning for ImageNet-A/R/V.
...
VERSION = "ocr-dota-v3-imagenet-avr-continuation-v1"
DATASETS = ("A", "R", "V")
STAGES = ("rank_refine", "update_refine", "base_refine")
DEFAULT_OUTPUT = REPO_ROOT / "log/all_dataset_perf/ocr_dota_v3_imagenet_avr_round2_fixed_order_8h_20260903"
...
def load_parent(parent: Path) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any], set[tuple[str, str]]]:
    ...
    history = core.load_jsonl(parent / "candidate_results.jsonl")
    historical = {(str(row["dataset"]), str(row["fingerprint"])) for row in history if row.get("status") == "ok"}
    parent_files = {name: core.sha256_file(parent / name) for name in required}
    return anchors, provenance, table, historical | {(d, table[d]["fingerprint"]) for d in DATASETS}
...
def generate_stage(dataset: str, stage: str, incumbent: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    if stage == "rank_refine":
        rank_spaces = {
            "A": (("prediction_strength", (.75, 1.25)), ("tau_rank", (.12, .18))),
            "R": (("prediction_strength", (.375, .625)), ("tau_rank", (.06, .09)),
                  ("update_power", (.25, .5, .75))),
            "V": (("prediction_strength", (.125, .25, .375, .625)), ("tau_rank", (.075, .30))),
        }
        for axis, values in rank_spaces[dataset]:
            specs.extend(spec(cfg, f"R_{axis}_{value:g}", **{f"rank__{axis}": value}) for value in values)
    elif stage == "update_refine":
        residual_values = {"A": (.125, .375), "R": (.0625, .125), "V": (0., .125)}[dataset]
        ...
        if dataset == "R":
            specs.extend(spec(cfg, f"U_beta_x{factor:g}", update__beta=min(1., max(0., beta * factor))) for factor in (.875, 1.125))
        else:
            specs.extend(spec(cfg, f"U_count_x{factor:g}", update__init_count=count * factor) for factor in (.875, 1.125))
    elif stage == "base_refine":
        axes = {"A": (("rho", (.95, .975, 1.025, 1.05)), ("eta", (.95, 1.05))),
                "R": (("rho", (.975, 1.025)), ("eta", (1.05, 1.10, 1.15, 1.20))),
                "V": (("rho", (.95, .975, 1.025, 1.05)),)}[dataset]
        for axis, factors in axes:
...
def build_identity(
    *, datasets: tuple[str, ...], stages: tuple[str, ...], global_seed: int,
    ...
    parent_run: str, historical_fingerprint_sha256: str,
) -> dict[str, Any]:
    identity = {
...
def main() -> int:
    ...
    anchors_all, provenance_all, parent_winners_all, historical_all = load_parent(parent)
    ...
    if args.dry_run:
        plan = {dataset: {stage: len([c for c in generate_stage(dataset, stage, {"config": anchors[dataset]})
            ...
            for stage_index, stage in enumerate(stages):
                ...
                for dataset_index, dataset in enumerate(eligible):
                    ...
                    if stage_block is None:
                        generated = generate_stage(dataset, stage, incumbent)
